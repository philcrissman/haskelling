...
main = do
  results <- sequence
    [ assertEqual "minimum Bool is False" (fst boolBounds) False
    , assertEqual "maximum Bool is True" (snd boolBounds) True
    , assertEqual "the full pair" boolBounds (False, True)
    ]
