...
main = do
  results <- sequence
    [ assertEqual "pairs numbers and letters" (zip' [1, 2, 3 :: Int] "abc") [(1, 'a'), (2, 'b'), (3, 'c')]
    , assertEqual "stops at the shorter list" (zip' [1, 2, 3, 4 :: Int] [True, False]) [(1, True), (2, False)]
    , assertEqual "empty when one is empty" (zip' ([] :: [Int]) "x") []
    ]
